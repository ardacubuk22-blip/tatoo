...
const WHATSAPP_NUMBER = "905557370933";
const WHATSAPP_TEXT = "Merhaba! Siteniz üzerinden yazıyorum.";
const INSTAGRAM_URL = "https://www.instagram.com/homeantiquehome";
...
function createFloatingLink({ kind, href, label, settingHref, waText }) {
  const link = document.createElement("a");
  link.className = `floating-btn floating-${kind}`;
  link.href = href;
  link.target = "_blank";
  link.rel = "noopener";
  link.title = label;
  link.setAttribute("aria-label", label);
  link.dataset.settingHref = settingHref;
  if (waText) link.dataset.waText = waText;

  link.innerHTML = `<svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="${ICONS[kind]}"/></svg>`;
  return link;
}

const floatingContact = document.createElement("div");
floatingContact.className = "floating-contact";
floatingContact.append(
  createFloatingLink({
    kind: "instagram",
    href: INSTAGRAM_URL,
    label: "Instagram sayfamız",
    settingHref: "instagram",
  }),
  createFloatingLink({
    kind: "whatsapp",
    href: `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(WHATSAPP_TEXT)}`,
    label: "WhatsApp'tan yazın",
    settingHref: "whatsapp",
    waText: WHATSAPP_TEXT,
  })
);
